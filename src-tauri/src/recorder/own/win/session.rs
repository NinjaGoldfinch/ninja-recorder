...
impl Recording {
    ...
    fn run(&mut self, device: &Device, origin: i64, commands: &Receiver<Command>) -> Ended {
        ...
        loop {
            ...
            if let Err(e) = self.write_due(origin, &mut k) {
                return Ended::Problem(lost_or(device, format!("{e} (at tick {k})")));
            }
    ...
    /// Finalizes the file. With the GPU device lost this still runs: the
    /// encoder's drain fails rather than waits, the sink writer returns that,
    /// and the fragments already on disk are the recording (`stop` bounds the
    /// wait regardless). The capture stops first, then the sink drains,
