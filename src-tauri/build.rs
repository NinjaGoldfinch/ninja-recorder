...
///
/// The own capture backend (`recorder/own/win/`) calls into `mfplat.dll` and
/// `mfreadwrite.dll`, and an ordinary import would make Windows refuse to
...
/// `own::win::device::media_foundation` asks whether the DLLs exist before
/// anything calls them.
///
