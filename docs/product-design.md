...
| Match metadata comes from the LCU after the game | Champion, KDA, outcome and mode come from Live Client Data *during* it | `fetch_match_summary` is implemented and unit-tested but still unwired: resolving *which* `gameId` just ended needs LCU behaviour no one has been able to check against a live client. The in-game API was already being polled at 1 Hz for markers and turned out to carry all four — as a champion *name*, so nothing has to resolve a champion id, and it works in Practice Tool and customs where match history does not. `queue`, `game_id`, `role` and `patch` have no live equivalent and stay `NULL` (#52, #53) |
