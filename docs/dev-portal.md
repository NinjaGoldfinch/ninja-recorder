...
| **Recorder** | The **daemon's** capture backend, which nothing else in the portal named: its `backend_name()` (`own (ready: <encoder>)`, `libobs (ready)`, or why it is unavailable), the `capture_backend` setting (`unset` when nothing is saved and the daemon picked), whether it is capturing, the file it is writing, and whether its capture worker process is up. All of it from `dev_health`, answered by the daemon (#282). No manual start or stop; see below |
...
same list through the directory scan that already finds `libobs-devtools.log`,
and so do a release build's `daemon.log`, `ui.log` and `libobs.log` when one is
installed beside it: they share the directory, but no longer a file.
