...
- **Key endpoints:**
  ...
  - `GET /lol-game-data/assets/v1/champion-summary.json` — the client's own asset store: `{id, name, alias, squarePortraitPath, roles}` per champion, plus an `id: -1` "None" sentinel. Turns the `championId` the two post-game endpoints answer with into a name.
