...
- **CI** ([`.github/workflows/ci.yml`](.github/workflows/ci.yml)) — one workflow, four jobs:
  - `test` runs `tsc --noEmit`, `cargo test` and `cargo clippy` on Windows only. The Rust half runs twice, once with `--features devtools` and once without: an off-by-default feature is otherwise never compiled by CI, and a broken `#[cfg]` would stay green until someone opened the portal. macOS was dropped from this job because the work is platform-independent and GitHub bills macOS runners at 10x against the free plan; `build` below still compiles macOS natively, so cfg'd breakage is still caught before a release.
  - `build` bundles installers natively per platform — NSIS on Windows, dmg on macOS, never cross-compiled — and uploads each as a workflow artifact (7-day retention), so a PR's own build is downloadable from its Actions run. It deliberately does *not* `needs: test`: the two share no output, and gating cost the whole test job in latency before the slow Windows bundle even started. Nothing unreviewed escapes, because a failed `test` fails the run either way. Pushes and manual runs add a third entry, the devtools bundle (§10).
  - `version` mints the release version *before* anything compiles, since it is baked into the binary, the installer filename and the About block. It is the commit's distance from the newest real tag, not "highest seen plus one" — a pure function of the commit, so simultaneous pushes cannot claim the same version and re-running a commit updates its own draft rather than minting a second.
  - `draft-release` publishes a draft GitHub Release from the bundles the same run just produced, for every commit that lands on `main`. Nothing is rebuilt and no tag is pushed: a draft holds its tag name without creating the ref, so the tag only exists once somebody publishes the draft from the Releases page — which is also what keeps `version` counting from a stable base. A `workflow_dispatch` run can opt in with `draft_release`, which is how the draft path gets exercised off `main`.
