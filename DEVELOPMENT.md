...
Finalizing stops the recorder, time-aligns the collected markers, writes the `recordings` row plus its `markers` and `samples` (§4), enforces retention (§6) and emits `library-changed`. It writes `duration_s` from the session clock and `champion`/`kda_*`/`win`/`game_mode` from the Live Client Data summary the session accumulated while the game ran. It still does **not** call `lcu::fetch_match_summary`, so `game_id`/`queue`/`role`/`patch` stay `NULL` on real recordings — resolving *which* `gameId` just finished needs LCU behaviour this machine can't verify live. The last finalized recording is also held in memory and exposed via `game_state_status`, so a failed DB write doesn't lose it.
...
- **`fetch_match_summary` is implemented, unit-tested, and called from nowhere** — which is why every `RecordingRow`'s `queue`/`role`/`patch`/`game_id` is NULL in practice. `champion`/`win`/`kda_*` no longer are: they come from Live Client Data during the game (§3.2). The portal at least makes the fetch runnable against a real client; wiring it into finalize is still open.
