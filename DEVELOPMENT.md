...
**Getting a build with it.** `npm run tauri:dev` for local work. CI also bundles a second Windows installer with `--features devtools`, uploaded as `ninja-recorder-devtools-windows-latest-<sha>` and attached to the draft release as `ninja-recorder-dev_<version>_x64-setup.exe` — for trying the portal on a machine that doesn't build from source (the Windows box, in practice). `tauri.devtools.conf.json` renames the product and binary to `ninja-recorder-dev` so it is a separate application to Windows. NSIS keys the uninstall entry, the default install directory and the shortcut off `productName`, so while the two shared one, this installer treated the real install as an older version of *itself* and uninstalled it first — a step that aborts the whole install with "Unable to uninstall!" if the old uninstaller returns non-zero or leaves the binary behind (a still-running app is enough). `mainBinaryName` splits the process name too, so neither build's "close the running app" check reaches across at the other; they now install side by side. The `identifier` is deliberately *not* overridden, so the portal still opens the library the real app writes to. **Those assets are draft-only and must be deleted before a release is published**: they carry raw SQL execution, arbitrary row writes, a database wipe and state-machine injection. `ci.yml`'s draft-release job writes a removal reminder into the draft notes for the same reason. The extra bundle runs on pushes to `main` only, since a second full Tauri build would roughly double the build job on every PR.
