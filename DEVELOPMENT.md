...
**Getting a build with it.** `npm run tauri:dev` for local work. CI also bundles a second installer per platform with `--features devtools`, uploaded as `ninja-recorder-devtools-<platform>-<sha>` and attached to the draft release as `*-devtools.exe` / `*-devtools.dmg` — for trying the portal on a machine that doesn't build from source (the Windows box, in practice). **Those assets are draft-only and must be deleted before a release is published**: they carry raw SQL execution, arbitrary row writes, a database wipe and state-machine injection. `release.yml` writes a removal reminder into the draft notes for the same reason. The extra bundle runs on pushes to `main` only, since a second full Tauri build would roughly double the build job on every PR.
