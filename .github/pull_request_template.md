...
<!-- What you actually ran, and on what. "CI is green" is the weakest form of
     ...
     strongest. Say which platform, because a Linux dev box does not compile
...
- [ ] If a command, an event or a boundary type changed, `src/lib/contract/`
      ...
      (`cargo run --features contract-gen --bin gen-contract`)
...
- [ ] No `{@html}` on any recording-derived string, and no new `escapeHtml`
      bypass, since `reconcile` imports whatever the user drops in the folder
