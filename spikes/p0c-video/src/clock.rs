...
//!
//! Everything here is a pure function of timestamps, so it is unit-tested on
//! any host. That is the plan's instruction for the spike (§4.5: "keep the
//! audio clock-alignment code testable off Windows"), and it is also what
//! stops the numbers the run prints from being a property of the machine the
//! arithmetic happened to run on.
//!
//! Two clocks meet in the file, and the question #8 asks is how far apart
//! they end up:
//!
//! - **Video** is placed on a constant 60 fps grid measured by the
//!   performance counter (QPC). Tick `k` is at `k / 60` seconds after the
//!   first captured frame, and its sample is whatever WGC last delivered.
//!   A tick with no new frame repeats the previous one, which is the plan's
//!   "cadence thread duplicating frames to hold 60 fps CFR".
...
//! audio stays on the video's clock. The correction is a stand-in for the
//! resampler `recorder/own/audio/` will need: crude, but it is enough to show
//! whether the drift is correctable and how big a correction it takes.
...
pub enum AudioClock {
    ...
    /// then trusted, which is what a naive pipeline does. The file's end
    /// offset is then the raw drift itself.
    Device,
}
